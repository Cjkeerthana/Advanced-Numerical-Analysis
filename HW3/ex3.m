...
for restart = [10,20,30,50,100]
    ...
    tic
    [x,flag,relres,iter,resvec]=gmres(@(x)A*(U\(L\x)),b,restart,tol,maxit);%Right preconditioning
    ...
    tic
    [x,flag,relres,iter,resvec]=gmres(@(x)L\(A*(U\x)),L\b,restart,tol,maxit);%Split preconditioning
...
figure(2)
semilogy(0:(resvec_R_length(1)-1),resvec_R(1:resvec_R_length(1))/norm(b),'-o')
hold on
from = 0;
for i = [2,3,4,5]
    from = from + resvec_R_length(i-1);
    to = from + resvec_R_length(i);
    semilogy(0:(resvec_R_length(i)-1),resvec_R(from+1:to)/norm(b),'-o')
end
yline(tol,'r--');
legend('10','20','30','50','100','tol')
xlabel('Iteration number')
...
figure(3)
semilogy(0:(resvec_S_length(1)-1),resvec_S(1:resvec_S_length(1))/norm(L\b),'-o')
hold on
from = 0;
for i = [2,3,4,5]
    from = from + resvec_S_length(i-1);
    to = from + resvec_S_length(i);
    semilogy(0:(resvec_S_length(i)-1),resvec_S(from+1:to)/norm(L\b),'-o')
end
yline(tol,'r--');
legend('10','20','30','50','100','tol')
xlabel('Iteration number')
